...
- `FeatureFlags.swift` — v1 release scope control: `static let` bools (`splat`, `portal`, `memoPins`, `furnitureGhosts`, all currently `false`) hide the UI entry points for those features while all implementation code stays compiled. Pins/ghosts are also filtered out of rendering (`RoomEntityFactory.makeRoomEntity` and `FloorPlanCanvas`) while their flags are off — the demo room contains both and is release-reachable on non-LiDAR devices. Re-enabling a feature = flip the bool (and review the user-facing delete/settings strings, which were reworded generically rather than flag-gated). Splat viewers etc. remain reachable via the DEBUG-only launch arguments above.

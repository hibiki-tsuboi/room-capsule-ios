...
いま目の前にある部屋を iPhone でスキャンして、**ミニチュア AR・実寸 AR・ポータル・写真のような 3D 空間・2D 間取り図**として保存し、あとから再体験できる iOS アプリ。
...
- **ポータル AR** — AR のドアの向こうに部屋が見える(遮蔽シェル使用)。ドアをタップすると光のバースト → 白フェードのカメラ演出で部屋の中へ
- **写真っぽく見る / スプラット AR** — Metal による Gaussian Splatting 実レンダリング。ビューアで眺めるほか、**現実空間にスプラットの部屋を置いて**中に入って歩ける
- **Splat 管理 / LiDAR キャプチャ** — 「この部屋をスプラット化(LiDAR)」で外部アプリなしに自分の部屋を Splat 化(法線推定つきの面スプラット・リアルタイムプレビュー付き)。`.ply` / `.splat` / `.spz` の取り込み・バージョン紐づけも
...
v1 では Gaussian Splatting・ポータル AR・メモピン・家具ゴーストの導線を `FeatureFlags.swift` で非表示にしていましたが、**v1 リリース後にすべて再有効化済み**です(スコープを絞りたいときは bool を戻すだけ)。
...
| ミニチュア / 実寸 / ポータル / スプラット AR | ARKit ワールドトラッキング対応端末 |
...
- `-autoSplat` / `-autoSplatAR` / `-autoSplatCapture` … Splat 系スタックのスモークテスト用(サンプル Splat 生成 → ビューア / スプラット AR / LiDAR キャプチャを自動表示)

## 使い方

1. ホームの「**部屋をスキャン**」で RoomPlan スキャン。iPhone をゆっくり動かして部屋全体を映し、完了したらデフォルトの名前のまま(または書き換えて)保存。
2. 部屋カードを開くと、サムネイル(タップで 3D プレビュー)・バージョン切替・採寸サマリーと、**3D プレビュー / ミニチュアで見る / ポータルを開く / 写真っぽく見る / スプラット AR / 時間を比べる / 図面で見る / メモを浮かべる / 家具ゴースト / Splat 管理**のグリッドが並びます。
3. 「バージョンを追加」で同じ部屋を時間をおいて再スキャンすると、「時間を比べる」で Before / After を行き来できます。
4. Splat 管理の「**サンプル Splat を生成**」で、実データがなくても Gaussian Splatting の実レンダリングをその場で体験できます。
...
└── Views/               ホーム / 詳細 / スキャン / ミニチュア(実寸トグル込み)/ ポータル /
                         3Dプレビュー / 間取り / タイムライン / メモ / ゴースト / Splat / 設定
```

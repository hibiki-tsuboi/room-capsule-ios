...
3. 部屋カードを開くと各モードへ:
   ...
   - **Splat 管理** — **「この部屋を簡易スキャン(LiDAR)」で外部アプリなしに自分の部屋を Splat 化**(深度 + カメラ映像から色付きの点を収集)。`.ply` / `.splat` / `.spz` の取り込み・バージョン紐づけ・削除、「サンプル Splat を生成」も
...
- **LiDAR 簡易スプラットスキャン**: LiDAR 深度 + カメラ映像から色付きの点を 2cm ボクセルで収集し(最大 60 万点)、等方ガウスの `.splat` として保存。学習(最適化)は行わない簡易方式なので、Scaniverse などの学習済み 3DGS よりは粗めです。
...
2. **LiDAR 簡易スキャン** — 部屋を見回して点が増えること、完了後にビューア / スプラット AR で色付きの部屋が見えること(色ズレ・位置ズレがないか)

...
- **写真っぽく見る** — Metal による Gaussian Splatting 実レンダリングのハブ画面。模型⇄写真のクロスフェード表示、「**AR で置く**」で現実空間にスプラットの部屋を設置して中に入って歩ける。データがなければその場で用意できる: 「この部屋をスキャンして作る(LiDAR)」で外部アプリなしに Splat 化(法線推定つきの面スプラット・リアルタイムプレビュー付き)、`.ply` / `.splat` / `.spz` の取り込み、サンプル生成。差し替え・削除・上下反転は画面内の「…」メニューから

...
1. ホームの「**部屋をスキャン**」で RoomPlan スキャン(非対応端末はデモモード誘導のフォールバックを表示。テスト用の「デモ部屋を追加」は設定画面の開発用セクションに仮置き中で、リリース前に削除予定)。
...
3. 部屋カードを開くと各モードへ:
   ...
   - **実寸で呼び出す** — 床タップで原点を決め 1:1 表示。透明度スライダー、ミニチュア⇄実寸切替。**ピンボタンで壁や家具をタップしてその場にメモピンを打てる**
   ...
   - **時間を比べる** — Before / After をスライダーでクロスフェード。**再生ボタンで自動ピンポン再生**
   - **図面で見る** — SwiftUI Canvas の 2D 間取り(寸法・凡例・ピンチズーム)
   - **メモを浮かべる** — 空間メモピン(カテゴリ・写真添付・3D プレビューでのタップ配置)
   - **家具ゴースト** — 半透明の家具プリセットを配置しサイズ/位置/回転を編集。ミニチュア AR・実寸 AR・3D プレビュー内では**ゴーストを指で掴んでドラッグ移動、ゴーストの上で 2 本指回転すると向きも変えられる**(自動保存)
...
- **Metal による実レンダリングを実装済み**(`Shaders/GaussianSplat.metal` + `MetalSplatView`)。
  ...
  - 奥→手前のアルファ合成順は 16bit カウンティングソートで作り、カメラが一定以上回転・移動したときだけバックグラウンドで再ソート。
  - **高次球面調和(SH 1〜3 次)対応**: 3DGS PLY の `f_rest_*` を Float16 で保持し、頂点シェーダで視線方向ごとに評価。角度によって色味・質感が変わる本家準拠の見た目になる(係数はチャンネルメジャー 45 half/スプラット、次数が低いファイルはゼロ詰め)。
...
- **LiDAR スプラットスキャン**: LiDAR 深度から法線を推定し、**面に沿った扁平ガウス**として書き出すサーフェル方式(1cm ボクセル・最大 100 万点・距離重み付き色・フローター除去つき)。**スキャン中は収集済みスプラットがその場でリアルタイム表示**され(目のボタンで ON/OFF)、どこが未スキャンか一目で分かります。学習(最適化)は行わないため、Scaniverse などの学習済み 3DGS には及ばないが、面が連続して見える品質になります。
...
- 高速にカメラを回すと一瞬だけ合成順が古いことがある(非同期ソートのため)。高次 SH 付きデータはメモリを多めに使う(100 万スプラットで約 +90MB)。LiDAR スキャン・.splat 形式は SH を持たないため DC 色のみ。
...
2. **LiDAR スプラットスキャン** — スキャン中のライブプレビューが現実にぴったり重なること(ズレたら要調整)、完了後に面が連続して見えること(壁・床がボテボテの球ではなく面に見えるか、色ズレがないか)
3. **実データの Gaussian Splatting** — Scaniverse 等で書き出した `.ply` を取り込み、視線依存色(SH)の効き方を確認(視点を動かすと光沢や色味が変わるか。バッジ横に「SH 3 次」表示が出るか)
3. 実寸 AR の「高品質」モードで透明度スライダーが効くこと(OpacityComponent 経由)
4. **ゴーストのドラッグ移動・回転** — ゴーストを掴んで動かす/ゴーストの上で 2 本指回転。離した位置・向きが保存され、部屋全体の回転と競合しないこと
5. **実寸 AR のメモピン直接配置** — ピンボタン ON → 壁・家具タップでエディタが開き、正しい位置にピンが保存されること
6. **タイムラインの自動再生** — 再生ボタンでスライダーがピンポン往復すること
7. **ポータルのトランジション** — ドアタップで光バースト → 白フェード → ドリーイン、閉じるときの白フェード

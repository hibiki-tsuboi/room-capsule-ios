...
- **部屋の中に入る** — 白フェードのカメラ演出で部屋の中へ入り、ドラッグで見回し・ピンチで前後移動して中を歩ける 3D ビュー(シミュレータでも動作)
...
| ミニチュア / 実寸 / スプラット AR | ARKit ワールドトラッキング対応端末 |
...
- `-autoDetail` … 最初のカプセルの詳細画面を自動表示(`-autoInside` を併用すると「部屋の中に入る」まで自動遷移)
...
2. 部屋カードを開くと、サムネイル(タップで 3D プレビュー)・バージョン切替・採寸サマリーと、**3D プレビュー / ミニチュアで見る / 部屋の中に入る / 写真っぽく見る / スプラット AR / 時間を比べる / 図面で見る / メモ管理 / 家具ゴースト / Splat 管理**のグリッドが並びます。
